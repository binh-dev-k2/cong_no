...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#modal-role').on('submit', 'form', function(e) {
            e.preventDefault();
            $(this).find('button[type="submit"]').attr('data-kt-indicator', "on");

            const body = {
                id: $(this).find('input[name="id"]').val(),
                role_name: $(this).find('select[name="role_name"]').val(),
            }

            // console.log(body);

            axios.post("{{ route('api.user.updateRole') }}", body, {
                    headers: {
