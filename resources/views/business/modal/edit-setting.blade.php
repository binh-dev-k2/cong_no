    ...
    <form action="#" id="form-edit-setting">
        ...
        <div class="modal-body bg-light">
            <div class="scroll-y me-n7 pe-7" style="max-height: calc(100vh - 25rem)">
                ...
                {{-- Empty State --}}
                @if (
                    (!isset($businessSettingMoney) || $businessSettingMoney->count() == 0) &&
        ...
        class BusinessSettingModal {
            ...
            addSettingBadges() {
                ...
                $('.business-setting-container').each(function() {
                    ...
                    const badgeClass = type === 'MONEY' ? 'bg-success text-white' :
            ...
            bindEvents() {
                ...
                $(document).off('click', '.delete-business-setting').on('click', '.delete-business-setting',
                    (e) => {
                        this.handleDeleteSetting(e);
                    });

                // Delete percent value
                $(document).off('click', '.delete-business-percent').on('click', '.delete-business-percent',
                    (e) => {
                        this.handleDeletePercent(e);
                    });

                // Add new business setting
                $(document).off('click', '.add-business-setting').on('click', '.add-business-setting', (
                    e) => {
                        this.handleAddSetting(e);
                    });

                // Add percent value
                $(document).off('click', '.add-business-percent').on('click', '.add-business-percent', (
                    e) => {
                        this.handleAddPercent(e);
                    });

                // Type radio change (only for new settings)
                $(document).off('change', '.type-radio:not(:disabled)').on('change',
                    '.type-radio:not(:disabled)', (e) => {
            ...
            handleTypeChange(e) {
                ...
                // Update badge
                const $badge = $(e.target).closest('.business-setting-container').find(
                    '.setting-type-badge');
